...
`packs/starter` ships with it — ground, loot, all ten monsters and the
creature itself, with totems, stalls and the named bosses left as letters so
the partial case is on screen rather than only in the documentation. Generated
by `tools/make_starter_pack.py`, which is also the worked example for making
your own.
